   ...
   if( $smof_data['rnr_enable_widescreen']) {
	...
	$count = 0;
		...
		if($count%$columns=='0' && $count!='1') {
			...
			if ( has_post_thumbnail()) {
				$blog_thumbnail= wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), $image_grid );
				$out .= '<a href="'.get_permalink().'" target="_blank" title="' . get_the_title() . '" class="blog-image"><img data-original="'.$blog_thumbnail[0].'" class="rnr-lazyload" src="'.$blog_thumbnail[0].'"/><div class="blog-overlay">';
			...
			$out .= '<div class="blog-item-description">
			            '.$blog_post_type.'<div class="post-details"> 
						<a href="'.get_permalink().'" target="_blank" title="' . get_the_title() . '"><h4>'.get_the_title() .'</h4></a>
						<span class="date">'.get_the_time('d').' '.get_the_time('M').' </span><span class="post-comments">'.get_comments_number().' '.__( 'Comments', 'rocknrolla' ) .'</span>
						<p>'.rocknrolla_limit_words(get_the_excerpt(), $excerpt_size).'</p><p><a href="'. get_permalink($post->ID) . '" target="_blank" class="read-more-link">' . '' . __('Read More', 'rocknrolla') . ' &rarr;' . '</a></p>
						</div>
